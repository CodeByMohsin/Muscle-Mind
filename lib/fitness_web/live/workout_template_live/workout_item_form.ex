defmodule FitnessWeb.WorkoutTemplateLive.WorkoutItemForm do
  ...
  def render(assigns) do
    ...
    <div class="p-8 bg-white rounded-lg">
      <h2 class="flex items-center justify-center pt-0 mb-4 text-3xl font-poppins">
        Add New Exercises
      </h2>
      ...
      >
        <div class="grid grid-cols-1 gap-2">
          <div>
            <label for="exercise_id" class="block mb-2 text-sm font-medium text-gray-700 font-poppins">
              Choose an exercise:
            </label>
            <%= select(
              f,
              :exercise_id,
              Enum.map(@exercises, fn exercise -> {exercise.name, exercise.id} end),
              value: @exercise_id,
              prompt: "Select an exercise",
              class:
                "form-select font-poppins block w-full rounded-md shadow-sm py-2 px-3 text-gray-700 bg-white focus:outline-none focus:ring focus:ring-blue-200 focus:border-blue-500 sm:text-sm"
            ) %>
          ...
          <div class="grid grid-cols-3 gap-2">
            <div>
              <label for="sets" class="block mb-2 text-sm font-medium text-gray-700 font-poppins">
                Number of sets:
              </label>
              <%= number_input(f, :sets,
                value: @sets_number,
                disabled: true,
                class: "form-input font-poppins rounded-md shadow-sm mt-1 block w-full"
              ) %>
              <%= error_tag(f, :sets) %>
            </div>

            <div>
              <label for="reps" class="block mb-2 text-sm font-medium text-gray-700 font-poppins">
                Number of reps:
              </label>
              <%= number_input(f, :reps,
                value: @current_reps,
            ...
            <div>
              <label for="weight" class="block mb-2 text-sm font-medium text-gray-700 font-poppins">
                Weight:
              </label>
              <div class="flex">
                ...
                <%= select(f, :weight_unit, ["kg", "lbs"],
                  class:
                    "form-select ml-2 rounded-md shadow-sm py-0 px-6 font-poppins text-gray-700 bg-white focus:outline-none focus:ring focus:ring-blue-200 focus:border-blue-500 sm:text-sm"
                ) %>
              </div>
            </div>
          </div>
        ...
        <div class="flex justify-center mt-6">
          <%= submit("ADD EXERCISE",
            ...
            class: "bg-green-400 hover:bg-green-600 text-white font-poppins py-2 px-4 rounded"
          ) %>
        </div>
      </.form>
    </div>
