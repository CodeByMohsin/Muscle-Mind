        ...
        <div class="p-6 mb-8 bg-purple-300 rounded-lg shadow-lg">
          <div class="p-8 mb-10 rounded-lg">
            <% list_of_same_exercise =
              Enum.group_by(@workout_template.workout_items, fn each -> each.exercise_id end)
              |> Map.values() %>
              ...
              <div class="p-4 mb-4 transition duration-200 transform bg-gray-100 rounded-lg shadow-md hover:scale-105">
                <div class="flex items-center justify-between mb-4">
                  <a href={"/exercises/#{workout_item_map.exercise_id}"}>
                    <h2 class="text-lg text-gray-800 font-poppins">
                      <%= Fitness.Exercises.get_exercise!(workout_item_map.exercise_id).name %> (<%= Fitness.Exercises.get_exercise!(
                        workout_item_map.exercise_id
                      ).body_part %>)
                    </h2>
                  </a>
                  <span class="px-2 py-1 text-sm font-semibold text-white bg-yellow-400 rounded-full hover:bg-yellow-600 font-poppins">
                    <%= length(each_list) %> sets
                  </span>
                </div>
                ...
                <table class="w-full">
                  ...
                  <tbody>
                    <%= for workout_item <- each_list do %>
                      <.form :let={f} for={@changeset} id={"created-workout-item-#{workout_item.id}"}>
                        <tr class=" font-poppins">
                          ...
                          <td class="py-2 text-gray-800">
                            <%= if @workout_start != :not_begin do %>
                              <label class="checkbox-btn">
                                ...
                                <%= checkbox(f, :check_box,
                                  value: workout_item.check_box,
                                  class:
                                    "form-checkbox h-5 w-5 rounded-full text-purple-500 transition duration-150 ease-in-out",
                                  phx_click: "workout_complete",
                                  phx_value_id: workout_item.id
                                ) %>
                            ...
                            <% else %>
                              <span>
                                <svg
                                  class="w-4 h-4 fill-current text-grey-400 hover:text-grey-600"
                                  version="1.0"
