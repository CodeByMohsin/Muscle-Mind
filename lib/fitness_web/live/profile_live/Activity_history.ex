  ...
  def render(assigns) do
    ...
    <div class="p-6 mb-8 bg-blue-100 shadow-lg">
      ...
      <%= if length(@workout_templates) >= 1 do %>
        <% user_template_owner =
          Enum.filter(@workout_templates, fn each -> each.user_id == assigns[:current_user].id end) %>

        <% workout_template_are_finished =
          Enum.filter(user_template_owner, fn each -> each.is_finished == true end) %>
        ...
        <div class="grid grid-cols-2 gap-4">
          <%= for workout_template <- Enum.reverse(workout_template_are_finished) do %>
            <div
              id={"workout_template-#{workout_template.id}"}
              ,
            ...
            >
              <div class="flex justify-between px-6 py-4 border-gray-400 rounded">
                ...
                <a
                  href={"/workout_templates/#{workout_template.id}"}
                  class="mb-2 text-xl text-gray-700 font-poppins"
                >
                ...
                <p class="relative inline-flex items-center justify-start px-6 py-3 overflow-hidden text-xl font-medium transition-all bg-white rounded font-poppins hover:bg-white group">
                  ...
                  <span class="relative w-full text-left text-black transition-colors duration-300 ease-in-out group-hover:text-white">
                    Score: <%= workout_template.workout_template_score %>
                  </span>
                </p>
              </div>
              ...
              <%= for each_list <- list_of_same_exercise do %>
                ...
                <div class="pb-1">
                  <span class="pt-4 pl-6 mr-2 text-base text-gray-500">
                    <%= length(each_list) %> ✕ <%= Fitness.Exercises.get_exercise!(
                      workout_item_map.exercise_id
                    ).name %> (<%= Fitness.Exercises.get_exercise!(workout_item_map.exercise_id).body_part %>)
                  </span>
                </div>
              <% end %>
            </div>
