          ...
          <% end %>
            <div class="flex items-center justify-between px-6 py-4 border-gray-400 user-info">
              <a href="#" class="mb-2 text-2xl text-gray-600 font-poppins hover:text-lime-500">
               ...
               <div class="absolute top-0 right-0 px-4 py-2 pt-2 text-xs font-bold text-white bg-gray-600 rounded-tr-lg rounded-bl-lg hover:text-xl">@<%= user.username %></div>
              </a>
            </div>
            <div class="flex justify-between ">
            <p class="pt-2 pb-2 pl-4 text-base text-orange-500"><%= Calendar.strftime(user.updated_at, "%B %-d, %Y") %></p>
            <% user_workout_template = Enum.filter(@workout_templates, fn each -> each.user_id == user.id and each.is_finished == true end) %>
              <span class="pb-2 pr-2 text-gray-500 "><%= length(user_workout_template) %> ✕ workout complete</span>
            </div>
