defmodule FitnessWeb.ScoreBoardLive do
  ...
  def render(assigns) do
    ...
    <div class="scoreboard bg-blue-100 rounded-lg shadow-lg p-6 mb-8">
      <h1 class="text-center text-3xl mb-4 font-poppins">Scoreboard</h1>
        ...
        <%= for {user, rank} <- rank do %>
          ...
          <% end %>
            ...
            <%= if user.id == @user_id do %>
              <p class="score-label font-poppins text-red-400 text-base">Total Score:</p>
              <p class="score font-bold text-3xl"><%= @player_score %></p>
              <% else %>
              <p class="score-label font-poppins text-red-400 text-base">Total Score:</p>
              <p class="score font-bold text-3xl"><%= user.player_score %></p>
              <% end %>
            </div>
          </div>
        <% end %>
  ...
  @impl true
  def handle_info({:update_users, {updated_users, updated_player_score, updated_user_id}}, socket) do
